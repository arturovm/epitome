...
func queryStringForRequest(req *http.Request, u *User, subsIds *[]string) string {
	...
	switch (*opts)["status"] {
	case "read":
		query = fmt.Sprintf("select * from (select articles.id, subscription_id, url, title, author, datetime(published) as published, body, body_type, summary, summary_type, coalesce(user_read_articles.read, 0) from articles inner join user_read_articles on articles.id = user_read_articles.article_id where subscription_id in (%s) and user_read_articles.user_id = %d %s %s order by datetime(published) desc limit %s) order by datetime(published) %s", strings.Join(*subsIds, ", "), u.Id, (*opts)["since_id"], (*opts)["before_id"], (*opts)["limit"], (*opts)["order"])
	case "unread":
	...
	case "all":
		query = fmt.Sprintf("select * from (select articles.id, subscription_id, url, title, author, datetime(published) as published, body, body_type, summary, summary_type, coalesce(user_read_articles.read, 0) from articles left outer join user_read_articles on articles.id = user_read_articles.article_id where subscription_id in (%s) and (user_read_articles.user_id = %d or user_read_articles.user_id is null) %s %s order by datetime(published) desc limit %s) order by datetime(published) %s", strings.Join(*subsIds, ", "), u.Id, (*opts)["since_id"], (*opts)["before_id"], (*opts)["limit"], (*opts)["order"])
	}
	return query
